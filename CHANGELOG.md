...
- 節を横に並べる種類 `weekly3`。見出し 2 を 4 つ書く前提で、
  `group_columns` が 1 つ目を横幅いっぱいの 1 列（トピックス）、
  2 〜 4 つ目を 3 列として組み立てる。**位置だけで決め、見出しの文字列は見ない**。
  4 つでなければ警告する（処理は続く。`--strict` で失敗扱い）。
  列の中に小見出し（`###`）を書くと、小見出しごとにまとめ直し、その中に列を並べる。
  ある分類が一部の期間にしか無い場合も、列の位置がずれないよう枠だけ残す。
  列の中は `####` でサブ項目に分けられ、サブ項目ごとにもう一段まとめ直して
  列を並べる。列の名前はサブ項目の行ごとに出す。
  件数の並びはサブ項目ごとにも、分類全体にも書ける。
  ルールを外せば、節の数を決めずにそのまま列にする並びにも戻せる。
  1 列の節は `topic_cards` が小見出し（`###`）ごとの枠に分ける
  （枠は縦に積み、枠の中は複数行可）。節そのものは枠を持たない。
  まとまりの見出しは帯（背景色）にし、列・サブ項目・トピックスの枠の見出しは
  一段弱い帯にして、どこからどの見出しの内容かが面で分かるようにした
- ルールが警告を出せるようにした（`rules.warn`）。変換結果の警告に混ざり、
  `--strict` の対象にもなる
  列が狭いぶん、表ではなく「1 行 1 件」で書けるようルールを 2 つ用意した。
  `count_summary` は `残:3 / 新規:1` だけの段落を件数の並びに、
  `entry_card` は `見出し｜属性｜説明` のリスト項目をカードにし、
  その下に字下げして書いた内容をコメントとして下段に入れる。
  普通の項目が混ざった箇条書きでは、箇条書きの体裁を外さない。
...
- `weekly3` の列の見出しに `（m/d〜m/d）` を自動で補う（`column_periods`）。
  1 つ目の列を front matter の期間とみなし、前後の列を 1 週間ずつずらす。
  md 側に日付を書く必要はない
- フッタの種類名を `{% block footer %}` で差し替えられるようにした。
  `weekly3` は見比べる紙面なのでフッタを出さない
