...
**リファレンス**
10. [同梱の種類](#同梱の種類)
11. [CLI オプション一覧](#cli-オプション一覧)
12. [出力先](#出力先)
13. [設定ファイル](#設定ファイル)
14. [ルール一覧](#ルール一覧)
15. [front matter のキー](#front-matter-のキー)
16. [Exit code](#exit-code)
17. [既知の制約](#既知の制約)
18. [ファイル構成](#ファイル構成)
...
Mermaid の図を書けます。**既定では無効**で、使う種類ごとに有効にします。

```yaml
# config.yaml
profiles:
  spec:
    mermaid: cdn          # インターネットから mermaid.js を読み込む
```
...
- **Mermaid は既定で無効** です。有効にする場合、`cdn` なら閲覧時にインターネット接続が要り、
