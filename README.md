...
`type: columns` は、**トップレベルの節をそのまま横の列にします**。
「前週 / 今週 / 来週の予定」のように、同じ形のものを並べて見比べる用途を想定しています。

Markdown 側は縦に書くだけです。`##` を 3 つ書けば 3 列になります
（節の数がそのまま列数になり、画面が狭ければ自動で折り返します）。
...
| `columns` | 節を横に並べる（前週・今週・来週など） | 件数の並び、1 行 1 件のカード、ステータスバッジ | 標準テーマ、広めの紙面 |
