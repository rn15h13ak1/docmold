...
- **スマートフォン・タブレットでも読める**: 狭い画面では余白を詰め、長い URL は折り返し、広い表だけを横スクロールにする
...
> **表の中だけは例外で、パイプにエスケープが要ります。** 表はセルの区切りを
> バッククォートより先に判定するため、`` `| ---: |` `` と書くとその行が余分な列に割れます。
> `` `\| ---: \|` `` と書いてください。**表示は `| ---: |` になります**（GitHub と同じく、
> 表の中のパイプに限ってエスケープを外します）。
...
生成物に**変換した端末の絶対パス**（ホームディレクトリの綴り）が混ざっていると、
どちらも終了コード 1 で止まります。生成物は追跡しているため、一度コミットすると
Git 履歴から消すのが難しく、ソースと違って中身を読む機会もないためです。
