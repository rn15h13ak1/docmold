...
1 つ目の節の箇条書きは、**項目ごとに枠が分かれます**。枠は縦に積まれ、
...
| `columns` | 見出し 2 を 4 つ。1 つ目が 1 列（トピックス）、2 〜 4 つ目が 3 列 | 件数の並び、1 行 1 件のカード、ステータスバッジ | 標準テーマ、広めの紙面 |
...
| `group_columns` | columns | 見出し 2 の 1 つ目を 1 列、2 つ目以降を列にする。列の中は小見出しごとにまとめ直す |
| `topic_cards` | columns | 1 列にした節の箇条書きを、項目ごとの枠に分ける（縦に積む） |
