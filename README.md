...
**リファレンス**
12. [同梱の種類](#同梱の種類)
13. [CLI オプション一覧](#cli-オプション一覧)
14. [出力先](#出力先)
15. [設定ファイル](#設定ファイル)
16. [ルール一覧](#ルール一覧)
17. [front matter のキー](#front-matter-のキー)
18. [Exit code](#exit-code)
19. [既知の制約](#既知の制約)
20. [ファイル構成](#ファイル構成)
...
`type: columns` は、**小見出しごとに、節を横の列にします**。
「バグ対応の前週 / 今週 / 来週」「改善要望の前週 / 今週 / 来週」のように、
同じ形のものを並べて見比べる用途を想定しています。

Markdown 側は縦に書くだけです。`##` に期間、`###` に分類を書くと、
**分類ごとのまとまりの中に、期間が列として並びます**。
...
`##` の数がそのまま列数になり、画面が狭ければ自動で折り返します。
ある分類が一部の期間にしか無い場合も、列の位置がずれないよう枠だけ残します。

**小見出し（`###`）を持たない節は、列にせず横幅いっぱいの 1 列で表示します。**
トピックスや前書きを、列の上（または下）に置くときに使ってください。

この節の箇条書きは、**項目ごとに枠が分かれます**（`topic_cards`）。枠は縦に積まれます。
項目の中は、空行を空けて書けば複数行になります。
...
枠を分けたくない場合は、番号付きリスト（`1.`）にするか、段落として書いてください。
どちらも枠にはしません。
...
> 表を使いたい場合はそのまま書けます（`status_badge` / `progress_bar` も効きます）。
> ただし列幅が狭いため、列が多い表は列の中で横スクロールになります。

#### 期間を外側にする（`group_columns` を外す）

小見出しでまとめ直さず、**節をそのまま列にする**こともできます
（「前週の中にバグ対応と改善要望」という並び）。`config.yaml` でルールを外します。
...
小見出し（`###`）を書かない文書では、`group_columns` は何もしません。
...
| `columns` | 小見出しごとに、節を横に並べる（前週・今週・来週など）。小見出しの無い節は 1 列 | 件数の並び、1 行 1 件のカード、ステータスバッジ | 標準テーマ、広めの紙面 |
...
| `entry_card` | columns | `｜` 区切りのリスト項目を 1 件のカードに変換 |
| `group_columns` | columns | 小見出しごとにまとめ直し、節を列として並べる（小見出しの無い節は 1 列のまま） |
| `topic_cards` | columns | 1 列として残った節の箇条書きを、項目ごとの枠に分ける（縦に積む） |
