...
`####` を書くと、**列の中がさらにサブ項目に分かれます**。区切りに使われるのは
いちばん浅い小見出し（`###`）だけなので、`####` は列の中身としてそのまま残ります。
...
枠の中は段落でも箇条書きでも、何行でも構いません。枠は縦に積まれます。
...
| `columns` | 見出し 2 を 4 つ。1 つ目が 1 列（トピックス。小見出しごとに枠）、2 〜 4 つ目が 3 列 | 件数の並び、1 行 1 件のカード、ステータスバッジ | 標準テーマ、広めの紙面 |
