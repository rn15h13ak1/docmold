...
索引 HTML は生成時刻を含み、実行のたびに差分が出てしまうため管理対象に含めない。
"""
...
def render(path: Path) -> str:
    ...
    config = load_config(default_search_dir=ROOT)
    return BANNER + convert_file(path, config).html
...
def main() -> int:
    ...
    if args.check:
        stale = stale_files()
        if not stale:
            print(f"サンプル HTML は最新です（{len(source_files())} 件）")
            return 0
    ...
    if changed:
        ...
        print(f"変更はありません（{len(source_files())} 件とも最新）。")
    return 0
