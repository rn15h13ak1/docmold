...
_SEVERITY_KEYS = ("重要度", "深刻度", "severity", "レベル", "影響度")
_TIME_COLUMNS = ("時刻", "日時", "時間", "time", "タイムスタンプ")
_TIMELINE_HEADINGS = ("時系列", "経過", "タイムライン", "timeline", "対応経過")
...
def severity_badge(soup: Any, meta: Dict[str, Any]) -> None:
    ...
    value = meta_lookup(meta, _SEVERITY_KEYS)
    ...
    for table in soup.find_all("table"):
        index = table_column_index(table, _SEVERITY_KEYS)
        if index is None:
            continue
...
def timeline_table(soup: Any, meta: Dict[str, Any]) -> None:
    ...
    tables = [t for t in soup.find_all("table") if table_column_index(t, _TIME_COLUMNS) is not None]
    if not tables:
        # 列名が無くても「時系列」節の最初の表はタイムラインとして扱う。
        for heading in find_headings(soup, _TIMELINE_HEADINGS):
    ...
    for table in tables:
        time_index = table_column_index(table, _TIME_COLUMNS)
